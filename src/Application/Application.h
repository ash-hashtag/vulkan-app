...
private:
	void initVulkan() {
		...
		LOGTST("initialising vulkan");
		createInstance();
		LOGTST("instance Created");
		window = glfwCreateWindow(WIDTH, HEIGHT, APP_NAME, nullptr, nullptr);
		glfwSetWindowUserPointer(window, this);
		glfwSetFramebufferSizeCallback(window, framebufferResizeCallback);
		LOGTST("opened window");
		createSurface();
		LOGTST("Surface Created");
		pickPhysicalDevices(); // pick GPU
		LOGTST("picked Physical Device");
		createLogicalDevice();
		LOGTST("Logical Device created");
		CreateSwapChain();
		LOGTST("Swap Chain Created");
		createImageViews();
		LOGTST("Image Views created");
		createRenderPass();
		LOGTST("Render Pass Created");
		createGraphicsPipeline();
		LOGTST("Graphics Pipeline created");
		createFrameBuffers();
		LOGTST("Frame Buffers created");
		createCommandPool();
		LOGTST("Command Pool created");
		createCommandBuffers();
		LOGTST("Command Buffers created");

		createSyncObjects();
		LOGTST("Synced Objects");
	}
	...
	void createInstance() {
		...
		if (func != nullptr) {
			...
			throw std::runtime_error("failed to setup debug messenger nullptr function");
		}
#endif
	...
	void pickPhysicalDevices() {
		...
		if (devicesCount == 0) {
			...
			for (const auto& device : devices) {
				if (rateDeviceSuitability(device)) {
	...
	bool rateDeviceSuitability(VkPhysicalDevice pdevice) {
	...
	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
		for (const auto& availableFormat : availableFormats) {
			if (availableFormat.format == VK_FORMAT_R8G8B8_SRGB && availableFormat.colorSpace == VK_COLORSPACE_SRGB_NONLINEAR_KHR) {
	...
	void createGraphicsPipeline() {
		...
		auto vertShaderModule = createShaderModule(vertexShaderCode);
		auto fragShaderModule = createShaderModule(fragmentShaderCode);
		...
		const VkPipelineShaderStageCreateInfo shaderStages[2] = { vertShaderStageInfo, fragShaderStageInfo };
	...
	void mainLoop() {
		...
		while (!glfwWindowShouldClose(window)) {
			...
			glfwPollEvents();
			drawFrame();
			//auto duration = (std::chrono::steady_clock::now() - start).count() * 0.000001;
			//printf("fps: %f\n", 1000 / duration);
		}
